...
// Streaming inference function for microphone audio
int inference_zipformer_model_streaming(rknn_zipformer_context_t *app_ctx, knf::OnlineFbank *fbank, VocabEntry *vocab, 
                                       std::vector<std::string> &recognized_text, std::vector<float> &timestamp, 
                                       int &frame_offset)
{
    int ret;
    ...
    int offset = N_OFFSET;
    int segment = N_SEGMENT;
    int fbank_frame_offset = frame_offset / ENCODER_OUTPUT_T * offset;
    
    // Process all available frames
    while ((num_frames - fbank_frame_offset) >= segment)
    {
        ret = get_kbank_frames(fbank, fbank_frame_offset, segment, encoder_input);
        if (ret < 0)
        {
            break;
        }

        ret = greedy_search(app_ctx, encoder_input, encoder_output, decoder_output, hyp, joiner_output, vocab, recognized_text, timestamp, fbank_frame_offset, frame_offset);
        if (ret < 0)
        {
            printf("greedy_search fail! ret=%d\n", ret);
            return ret;
        }
        fbank_frame_offset += offset;
    }
